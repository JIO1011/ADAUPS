        ...
        <AnimateOnScroll>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
            ...
            {benefit.images && benefit.images.length > 0 && (
              <div className="hidden md:block h-16 w-32 relative">
                <img src={benefit.images[0]} alt={`${benefit.title} logo`} className="h-full w-full object-contain mix-blend-multiply" referrerPolicy="no-referrer" />
              </div>
            )}
          </div>
          <p className="text-lg text-slate-700 leading-relaxed">{benefit.description}</p>
          </div>
              ...
              <section className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {benefit.images.slice(1).map((img, index) => (
                    <img key={index} src={img} alt={`Detalle ${index + 1}`} className="rounded-lg w-full object-contain border border-slate-100" referrerPolicy="no-referrer" />
                  ))}
                </div>
              </section>
            )}
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 rounded-2xl p-6 shadow-sm text-white">
              ...
              <p className="text-slate-200 text-sm leading-relaxed">{benefit.howToUse}</p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              ...
              <p className="text-slate-700 text-sm leading-relaxed">{benefit.locations}</p>
            </div>

            {benefit.documents && benefit.documents.length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-500 mb-4">Documentos</h3>
                <ul className="space-y-3">
                  {benefit.documents.map((doc, index) => (
                    <li key={index}>
                      <a href={doc.url} target="_blank" rel="noopener noreferrer" className="flex items-start text-sm text-blue-600 hover:text-blue-800 transition-colors group">
            ...
            <div className="bg-blue-50 rounded-2xl p-6 shadow-sm border border-blue-100">
              ...
              <p className="text-blue-900 text-sm leading-relaxed">{benefit.contact}</p>
            </div>
            </div>
          </div>
        </AnimateOnScroll>
